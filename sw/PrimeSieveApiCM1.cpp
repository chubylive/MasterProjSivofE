...
PrimeSieveApiCM1::PrimeSieveApiCM1(uint16_t pe, uint16_t rows) : NUMBER_OF_PE(pe <= 280 ? pe : 280),
...
unsigned *PrimeSieveApiCM1::pe_command_register_vaddress(unsigned pe_id, unsigned register_id)
{
    unsigned * vaddress;
...
unsigned *PrimeSieveApiCM1::pe_data_register_vaddress (unsigned pe_id) {
    unsigned * vaddress;
...
unsigned *PrimeSieveApiCM1::pe_status_register_vaddress (unsigned pe_id) {
    unsigned * vaddress;
...
void PrimeSieveApiCM1::write_command_register_0 (unsigned pe_id, unsigned argument_2, unsigned argument_3    ) {
    unsigned * vaddress = pe_command_register_vaddress (pe_id, 0);
    * vaddress = argument_2 | (argument_3 << 16);
}

void PrimeSieveApiCM1::write_command_register_1 (unsigned pe_id, unsigned command   , unsigned argument_1) {
    unsigned * vaddress = pe_command_register_vaddress (pe_id, 1);
    * vaddress = command    | (argument_1 <<  4);
}

unsigned PrimeSieveApiCM1::read_command_register (unsigned pe_id, unsigned register_id) {
...
unsigned PrimeSieveApiCM1::read_data_register (unsigned pe_id) {
    unsigned * vaddress = pe_data_register_vaddress (pe_id);
    unsigned register_read_data = * vaddress;
    return register_read_data;
}

unsigned PrimeSieveApiCM1::read_status_register (unsigned pe_id) {
...
void PrimeSieveApiCM1::clear_memory(unsigned pe_id)
{
    write_command_register_1 (pe_id, PE_CLEAR_MEMORY_COMMAND);
}

void PrimeSieveApiCM1::clear_memory_all() {
...
unsigned PrimeSieveApiCM1::read_memory_row(unsigned pe_id, unsigned row) {
...
void PrimeSieveApiCM1::read_memory(unsigned pe_id) {
...
void PrimeSieveApiCM1::read_memory_all()
{
    for (unsigned pe_id = 0; pe_id < NUMBER_OF_PE; pe_id++)
    {
        read_memory(pe_id);
...
void PrimeSieveApiCM1::compute(unsigned pe_id, unsigned step, unsigned first_offset)
{
    write_command_register_0 (pe_id, first_offset, MAX_OFFSET);
    write_command_register_1 (pe_id, PE_COMPUTE_COMMAND, step);
}

unsigned PrimeSieveApiCM1::compute_for_current_prime(unsigned current_prime)
{
...
unsigned PrimeSieveApiCM1::get_next_prime(unsigned current_prime)
{
...
void PrimeSieveApiCM1::compute_all()
{
...
std::stringstream PrimeSieveApiCM1::read_memory_n(uint32_t n)
{
...
void PrimeSieveApiCM1::compute_prime(uint32_t n)
{
...
float PrimeSieveApiCM1::time_compute(uint32_t n)
{
    clock_t start, end;
    clear_memory_all();
    FILE * fp;
