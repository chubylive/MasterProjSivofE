...
//      device_base_vaddress [2] = (unsigned *) getvaddr (BASE_ADDRESS_2);
        ...
        start = clock();
        compute_all      ();
        // gettimeofday(&stop_us, NULL);
        end = clock();
        // printf("compute all took %lu us\n", (stop_us.tv_sec - start_us.tv_sec) * 1000000 + stop_us.tv_usec - start_us.tv_usec); 
        cpu_time = ((double) (end - start))/ CLOCKS_PER_SEC;
        printf("read all took: %f seconds\n", cpu_time);
        start = clock();
        read_memory_all  ();
        end = clock();
        cpu_time = ((double) (end - start))/ CLOCKS_PER_SEC;
